...
- Course Flow resolves collisions between saved and newly automatic unit positions. Dense source-less lecture Blueprints use one layout-only root and a compact teach-then-check hierarchy so topic branches remain ordered and readable without changing persisted relationship semantics.
