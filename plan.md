...
- Render a typed course system in React Flow: topics and concepts connected by `contains`, `requires`, `teaches`, `assesses`, `remediates_to`, `next`, and `cites` relations. Support course/topic/concept semantic zoom, a keyboard-accessible outline, persistent manual positions, and ELK.js auto-layout for only the visible subgraph.
- Add a synchronized inspector for topic/concept/clip/question/source/relationship records. In Design mode instructors can update inspector fields, connect or reconnect prerequisites, and edit course sequence. AI-authored changes remain typed proposals with `Accept AI suggestion / Edit manually / Dismiss`; direct teacher edits remain audited and open a private working revision on first save.
...
- Repository/service/API tests for typed Blueprint nodes/edges, revision isolation, ownership/privacy, evidence overlays, sequence editing, learner-path preview, and single-task proposal-pack retrieval.
...
- Frontend unit/integration tests for mode switching, semantic zoom, outline/canvas parity, inspector editing, relationship manipulation, proposal review, learner path rendering, and empty/loading/error states.
