...
**Deliverables:**
- Replace the separate published Overview and Course Map destinations with one Blueprint workspace whose published evidence state is implicit rather than a mode tab. Provide an explicit **Edit Blueprint / Done editing** toggle for direct structural work, a dedicated **Preview learner journey** action for cohort/learner simulation, and a conditional **Review changes · N** action that appears only for a real private revision diff. Retain Assessments, Preview, and Settings initially as focused bulk/deep editors reachable from Blueprint.
- Render a typed course system in React Flow: a deterministic responsive topic-swimlane whole-course view with sequenced concept bundles and synchronized aliases for shared concepts; bounded topic/concept focus that progressively reveals clips, questions, remediation, sources, and typed relations; and a concept-only dependency lens using fixed-port orthogonal ELK layout. Keep the previous all-artifact graph fully functional under `Old Blueprint` until the revised human checklist is confirmed, without letting structured layout overwrite its saved positions.
- Add a synchronized inspector for topic/concept/clip/question/source/relationship records. While **Edit Blueprint** is active, instructors can update inspector fields, connect or reconnect prerequisites, and change course sequence through constrained drag or accessible controls. Cross-topic drops require an explicit add-versus-move confirmation for reviewed multi-topic links. AI-authored changes remain typed proposals with `Accept AI suggestion / Edit manually / Dismiss`; direct teacher edits remain audited and open a private working revision on first save.
...
- Repository/service/API tests for typed Blueprint nodes/edges, revision isolation, ownership/privacy, evidence overlays, sequence editing, working-revision-safe multi-topic concept links, learner-path preview, and single-task proposal-pack retrieval.
...
- Specialist tests proving one evidence brief can create a one-to-six proposal pack, every proposal has an exact typed before/after diff and citation/evidence provenance, partial failure is visible, and no learner-facing record changes before an atomic Accept/Edit action plus publication.
- Frontend unit/integration tests for deterministic swimlane construction, shared-concept aliases, artifact summaries, action/lens switching, semantic focus, outline/canvas parity, constrained sequence/topic drag, inspector editing, relationship manipulation, proposal review, conditional revision review, Old Blueprint parity, learner journey preview, and empty/loading/error states.
...
**Human test checklist:**
- [ ] Open a realistic published course and confirm it lands directly on the current course evidence without a redundant Live tab. Enter and leave `Edit Blueprint`, open and close `Preview learner journey`, and—when changes exist—open `Review changes`; confirm the selected topic/concept and inspection lens remain stable.
- [ ] With `Edit Blueprint` active, add or reconnect a prerequisite, reorder two concepts, edit one inspector field, and confirm each direct edit is audited in a private working revision while the active learner course remains unchanged.
- [ ] Select a struggling concept in the default Blueprint, inspect the learner evidence and cited source/clip/question coverage, ask the course team to prepare a misconception-recovery improvement, and confirm the resulting coordinated pack is coherent.
...
- [ ] Navigate the complete Blueprint and learner path with keyboard only, verify the outline offers equivalent non-canvas operations, and confirm labels/statuses remain understandable with a screen reader.
...
- [ ] In the new whole-course Blueprint, scan topic lanes without zooming out; confirm concept sequence, coverage, evidence, shared concepts, and urgent gaps are understandable at a glance, then use topic focus, concept focus, and Dependencies without losing context.
- [ ] With `Edit Blueprint` active, drag a concept within its lane and confirm the reviewed global sequence changes. Drag a shared concept across topics, exercise both `Also link` and `Move`, and confirm each action creates only a private audited revision while the published learner path remains unchanged.
- [ ] Open `Old Blueprint`, confirm its Live/Design/Learner path/Revision modes and free-form saved positions still work, and verify changes made there remain synchronized with the same reviewed course artifacts.
