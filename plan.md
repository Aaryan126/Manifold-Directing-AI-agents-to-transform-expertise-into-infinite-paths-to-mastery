...
- When lecture generation completes, open its private Blueprint directly in Live with no completion banner. Scope concept-graph generation to that lecture video’s topics so adding a lecture cannot change prior lecture membership. For large Blueprints, keep the complete graph mounted but initially frame the first topic’s direct teaching/check neighborhood at readable scale; the existing fit-all control reveals the whole lecture. For a pending Course Director relationship removal/reconnection, retain the entire current graph and visually mark the exact existing edge as a private proposed change before `Accept / Edit / Dismiss`; proposal arrival itself must not mutate topology or camera.
...
- [ ] Open a large lecture Blueprint and confirm it starts at a readable first-topic teaching/check neighborhood while the complete graph remains pannable; use the fit-all control and confirm the whole lecture appears, then use topic focus, `Jump to` selection, pan/zoom, and saved positions without losing context.
