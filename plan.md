...
- Competition evaluation and decision-lineage evidence: persist per-generation-task wall time and provider-returned usage; provide a reproducible harness that runs tests, starts the production stack, benchmarks warm endpoints, optionally performs and cleans up a disposable real-provider course compilation, calculates model cost from dated official pricing, and emits machine-readable plus judge-readable reports. Add an on-demand Blueprint Live `Trace decision` view spanning source → concept → clip → assessment → learner evidence → route event → dashboard signal → signal-linked private proposal; unavailable stages must be explicit rather than inferred. A published Live trace may resolve that final proposal only through the exact signal ID into the active revision's current unpublished child revision, keeping the proposal visible, private, and unapplied.
- Judge-facing competition artifacts: maintain a current README, a maximum-1,000-word write-up organized under the five competition pillars, and a rough three-minute demo outline. Capability comparisons may use current primary vendor/industry sources, but vendor speed claims must not be presented as independent measurements and a traditional/manual time comparison remains unclaimed until the same source is built through a matched workflow.
...
- Competition harness regression coverage for usage parsing, durable task measurements, fresh-course generation upsert correctness, decision-trace ownership/revision scoping, API serialization of available/missing stages, disposable-course cleanup, and generated-report schema.
...
**Deliverables:**
- Replace the separate published Overview and Course Map destinations with one detailed hierarchical Blueprint workspace and two purposeful modes: **Live** for the current read-only course structure and its evidence, and **Design** for direct private editing of the working revision. Course and lecture entry default to Live; when no active revision exists, Live truthfully renders the private working draft rather than an empty graph. Inside a selected lecture, Blueprint is the implicit workspace rather than a selected header tab. One slim course-title header holds revision/publication state, actionable course health, Sources, publication, and an overflow for focused assessment review, learner preview, and course settings. The graph begins immediately beneath that header; Live/Design plus mode-aware navigation, relationship, evidence, layout, add, order, and undo actions live in one compact floating canvas toolbar instead of persistent metric and relationship rows. Keep learner experience in the dedicated Preview/learner workspace and exact live-versus-working comparison in the publication/revision review flow rather than duplicating those tasks as canvas modes.
- Render the complete typed course system in React Flow: topics, concepts, clips, questions, remediation, sources, and typed relations share one inspectable graph. Fit the visible graph on initial load, preserve instructor-saved positions, and allow topic-neighborhood focus. In Live, selecting an artifact temporarily re-arranges only that artifact and its first-degree non-dismissed incoming/outgoing neighbors, keeps every incident explanatory edge visible regardless of filters, and lets neighbor selection move the focus; the inspector close control or empty-canvas click restores the complete lecture. The clicked artifact anchors the target camera near the center of the usable graph region beside the measured inspector; zoom fits the full first-degree neighborhood and translation clamps only enough to retain every relative with a safe gutter. Capture the selected node's exact rendered center in the click handler before changing topology. During the transition, synchronously commit each controlled React Flow node/edge presentation and derive the matching camera translation from the focal node's interpolated graph center, keeping that artifact on one direct screen-space path from its clicked position to the final anchor rather than letting a stale rendered layout and a newer camera frame visit another section. These Live neighborhood changes keep one React Flow canvas mounted while ELK prepares the target projection, then drive retained-node coordinates, entering/exiting node and edge opacity, dynamic edge paths, and the viewport from the current painted frame through one interruptible Motion spring. They never replace the graph with a blocking loader or issue a second completed-layout fit, stale layouts remain cancellable with the latest request winning, and reduced-motion users receive the final frame immediately. Design selection retains the complete graph and saved authoring context. Provide the synchronized topic/concept outline as an on-demand, keyboard-operable `Jump to` popover so instructors retain a readable non-canvas navigation path without permanently reducing graph width.
- Give the graph a consistent tree grammar: source → topic → concept → teaching clip/check is the sole automatic hierarchy spine, while prerequisite, sequence, remediation, and citation relations render as secondary cross-links without changing ranks. Use distinct accessible node components for every artifact kind, modest content-aware width growth within type-specific caps, more flexible height growth that shows complete titles, deterministic top-to-bottom semantic layers, stable sibling order, fixed relation-specific ports, ELK orthogonal edge routes, saved-position collision resolution, and sequence-aware crossing minimization. Show the instructional core by default; reveal citations and remediation contextually or through explicit relationship filters. Design mode keeps deliberate saved manual positions after overlap resolution and offers a presentation-only Auto arrange action that restores the hierarchy.
- Keep exactly one artifact-inspector shell mounted while a Live selection moves between visible neighbors; update its accessible label and contents in place so an exiting inspector cannot stack behind the new selection. Empty-pane dismissal, the inspector close control, and any return to the whole-course view must physically remove that shell immediately rather than leaving an inert exit-presence panel painted over the restored graph. Route transient/fallback relationships from their actual top, bottom, left, or right handles, and leave bounded clearance before marker-bearing target ports so arrowheads retain the correct approach direction and remain visible.
- Add a synchronized inspector and contextual node/edge toolbar for topic/concept/clip/question/source/relationship records. Clip inspectors play the exact bounded teaching moment in context. In **Design** mode instructors can add, update, connect/reconnect, reposition, and safely remove supported artifacts through type-aware direct controls and the accessible `Jump to`/inspector equivalent. One consolidated `Add node` action reveals the supported artifact kinds without permanently occupying the graph toolbar. Hovering or keyboard-focusing a connectable node reveals four prominent external `+` controls, each joined to one side by a short directional stem; choosing one asks for the relationship meaning, then turns the canvas into a valid-target picker with clear cancellation and invalid-target explanations. Edge selection exposes relation meaning, endpoints, reconnect, and traceable removal.
...
- Give Course Director parity with the validated manual Design vocabulary. It may answer questions or prepare an ordered, bounded change plan containing supported node add/edit/remove—including a concept-grounded question with its confidence check and remediation fallback—and relationship add/reconnect/remove operations against exact current Blueprint identities. The response gives a concise systematic account of every proposed operation and its rationale. Each AI operation remains an independent private proposal with `Accept / Edit / Dismiss`; accepting invokes the same server-side validation/audit path as the equivalent manual action, records a durable inverse, and enables the shared Design Undo control to restore that accepted/edited private change and mark its proposal `undone`. Unsupported or ambiguous requests produce clarification rather than an invented mutation. Course Director receives both the private working Blueprint and the published-only artifacts missing from it, so a repeated request to remove an already-private-removed artifact explains the Live/Design distinction rather than targeting a similarly named record. Preparing proposals does not refresh or rearrange the map before a decision. Accepting or editing a structural proposal preserves the current Live/Design mode, reloads the revision-aware workspaces, and keeps the last complete graph painted under a restrained Motion updating state until the replacement topology and viewport are ready; the canvas may never expose an empty or unrelated intermediate frame.
- When lecture generation completes, open that newly generated lecture directly in its private Design Blueprint with no completion banner; it does not exist in the published Live revision until the instructor publishes updates. If generation completes while the instructor is away or the page reloads, the first later course open resumes that same Design handoff once, then records it as seen so ordinary later course and lecture entry returns to the Live default. Contextual navigation from a Course Flow mode preserves the selected Live/Design revision. Scope concept-graph generation to that lecture video’s topics so adding a lecture cannot change prior lecture membership. Keep the complete graph mounted, initially frame the source/topic backbone, and on the lecture's first session opening briefly reveal topics, then concepts, then teaching moments/checks without changing topology. The reveal is skippable, completes immediately under reduced motion, and does not replay during the session; the existing fit-all control reveals the whole lecture. For a pending Course Director relationship removal/reconnection, retain the entire current graph and visually mark the exact existing edge as a private proposed change before `Accept / Edit / Dismiss`; proposal arrival itself must not mutate topology or camera.
- For the exact YAML-configured Business 101 recording course only, Accept/Edit from a focused Live neighborhood may switch the mounted graph to a clearly labelled, session-only `Private preview` of the working revision without changing the mode control. Preserve the focal logical artifact when it survives so deleted/added/reconnected neighbors update in place; closing focus shows the same working topology in the full map. Reload and publication clear the preview, the active revision remains authoritative for learners, and ordinary courses retain the published-Live contract.
...
- Redesign the learner course as a concept-driven path: compact prerequisite/current/next strip, one current learning activity, embedded assessment checkpoint, route explanation, optional mastery map, and cited learner aids. Post-answer adaptation must expose the saved correctness/confidence evidence, persisted route reason, and exact next reviewed step in one decision card; timeline labels describe the actual action/artifact rather than repeating concept names. Mastery Map renders the current/recommended neighborhood by default and offers a deliberate whole-course toggle. The active published revision and existing enrollment/privacy gates remain authoritative; explicitly simulated competition learners may access their exact enrolled rehearsal revision without making drafts available to ordinary learners.
...
- Specialist and Course Director tests proving one evidence brief, optional post-edit cleanup request, or conversational Blueprint command can create a one-to-six ordered proposal pack; every proposal has an exact typed before/after diff and citation/evidence provenance; unsupported/ambiguous operations are rejected; partial failure is visible; accepted/edited proposal inverses are durable and audited; Undo restores the private revision and proposal ledger to an explicit `undone` state; and no learner-facing record changes before publication.
- Frontend unit/integration tests for complete typed graph construction, artifact-to-node/port mapping, the consolidated Add node menu, four-sided hover ports, relationship-kind choice, valid-target selection/cancellation, contextual node/edge actions, guided add/remove, deterministic impact preview, scoped learning order, core/all/contextual relation visibility, first-degree Live neighborhood isolation/restoration, single-canvas multi-frame neighborhood motion without a blocking loader or second fit, interrupted-focus latest-request behavior, reduced-motion fallback, ELK bend-point consumption, deterministic semantic ordering, saved-position layout, single-settle guided opening camera plus explicit fit-all, stable Live/Design switching, proposal-arrival map stability, accepted-proposal mode preservation and retained-map topology handoff, topic focus, `Jump to`/canvas parity, node movement, in-context clip playback, inspector editing, relationship manipulation, optional AI cleanup, typed Course Director plans, proposal review, header course-health disclosure, overflow access to focused assessment/preview/settings workspaces, and empty/loading/error states.
...
**Human test checklist:**
- [ ] Open a realistic published course and move between Live and Design without losing the selected topic/concept; confirm the graph makes clips, questions, sources, sequence, evidence, and routing feel like parts of one course system rather than separate tools. Open the assessment workspace and `View as learner` from the header overflow, then return to the map and confirm those focused tasks remain clear without becoming permanent lecture tabs.
...
- [ ] Ask Course Director to add and edit a node, remove a node, add a remediation connection, and reconnect an existing relationship. Confirm its response lists the exact ordered plan, unsupported ambiguity is surfaced instead of guessed, every operation has its own `Accept / Edit / Dismiss`, and accepted operations match the equivalent manual Design actions without changing the published learner course. After accepting one change, use the toolbar Undo control and confirm the exact private change is restored, the proposal reads `undone`, and the published learner course remains unchanged.
...
- [ ] Navigate the complete Blueprint and learner preview with keyboard only, verify the `Jump to` outline popover offers equivalent non-canvas navigation, and confirm labels/statuses remain understandable with a screen reader.
...
- [ ] Open a large lecture Blueprint in a fresh browser session and confirm its source/topic backbone appears first, concepts unfold next, and teaching moments/checks resolve last without a loading flash or topology jump. Confirm `Skip reveal` goes directly to the final graph, reopening the lecture in the same session does not replay it, and reduced motion receives the final state immediately. Use fit-all, topic focus, `Jump to`, pan/zoom, and saved positions without losing context.
- [ ] In Live, select a topic, concept, clip, question, and source in turn; confirm only the selected artifact and its directly connected neighbors remain, retained nodes visibly travel into their new positions without a loading flash, selecting a neighbor moves the focus, and both the inspector cross and an empty-canvas click smoothly restore the complete lecture. Interrupt one transition with a quick close and confirm the latest requested view wins. Enable reduced motion and confirm the same states change immediately. Switch to Design and confirm selection no longer hides the wider editing context.
- [ ] In Design mode, drag a node and confirm its position persists after reload. Use the keyboard-operable sequence controls and the `Jump to`/inspector path for the same semantic edits; confirm presentation-only movement does not change the published learner path and structural edits remain in the private working revision.
...
- Existing videos backfilled as lecture units. New lecture generation is video-scoped, does not rename an established course, and refreshes the cross-course concept graph without duplicating other lectures' clips or questions. Starting generation on a published course with no private revision atomically opens/reuses the working revision before the run is created; this internal prerequisite is never exposed as an instructor error.
- Course Flow React Flow workspace as the sole course-level structural destination, with Live/Design, compact ungrouped sequences, module swimlanes only when modules exist, typed nodes/relationships, a persistent `New lecture` action, add/edit/remove, impact review, undo, auto-arrange, and contextual lecture-to-Blueprint navigation. Its initial camera is unit-count-aware—close for one unit, standard for two-to-four, and overview scale for five-plus—without restricting subsequent manual zoom; the header balances its right action cluster against the complete left title/copy block and gives the primary action plus Live/Design control a consistent 48px minimum height. Opening a course stages only the header, contextual authoring strip, and graph while React Flow eases into the selected camera. Opening a lecture uses the existing Motion for React vocabulary for one short keyed Course Flow-to-Blueprint handoff, persistent-header title crossfade, lecture-tab reveal, and staged Blueprint command/controls/body arrival; reduced-motion users receive immediate final states.
- Ungrouped Course Flow units always use the plain compact sequence, even beyond two units; lane containers are reserved for real persisted modules. An exact-course YAML-gated Business 101 recording rehearsal replays its cached second lecture through the normal durable progress, private Blueprint, and explicit publication path, while disabled/non-matching operation remains provider-backed. A dry-run-by-default reset command restores the one-lecture recording baseline.
- Course Flow resolves collisions between saved and newly automatic unit positions. Lecture Blueprints retain the hierarchy-first ELK structure: independent topic → concept → teaching/check subtrees use final content-aware node dimensions and orthogonal semantic routes, without a synthetic common root or dense-only relationship rewrite. For an unsaved source-less map, only the intact connected-component rectangles are compacted into reading-order shelves; internal hierarchy geometry and relationships do not change. Instructor-saved positions remain authoritative.
...
- Lecture-focused Blueprint opened contextually from a lecture node. The compact header title becomes the lecture title and Blueprint becomes the implicit full-height lecture workspace without a redundant selected tab. Focused assessment review, `View as learner`, and course settings open from the header overflow; their header supplies a clear return-to-map action. The back control returns to Course Flow without a duplicate breadcrumb row. A secondary `Cross-lecture concept map` appears only when multiple lectures make shared concepts and cross-lecture prerequisites distinct.
- Course Flow is the implicit sole course-level destination and therefore does not render a redundant selected navigation tab; course-wide routing settings remain a compact gear popover. A selected lecture likewise omits Blueprint/Assessments/Preview destination tabs in favor of the graph-native Blueprint, contextual node actions, and the focused-workspace overflow. Course Flow relationships use contextual node-edge `+` controls followed by relationship meaning and target selection; no separate generic `Add relationship` toolbar action remains. Course Director streams progress and Markdown responses into a course-scoped runtime transcript retained through close/reopen and client-side navigation during the current login but reset by login, logout, full reload, or container-driven reload. A clean transcript renders a course-specific presentation welcome rather than an empty dock, without a separate workspace-context label; all generated mutations retain durable independent `Accept / Edit / Dismiss` checkpoints.
...
- Course Director context and typed proposals spanning both Course Flow and detailed Blueprint, using Agnes 2.5 Flash when selected (with the existing OpenAI/local fallback), the same manual mutation services, closed-schema local validation, and independent `Accept / Edit / Dismiss` review.
...
- UI tests cover module grouping, lecture focus/Whole course, node inspectors, relationship authoring, undo, graph refresh, non-empty topology transitions, graph-native header/toolbar/`Jump to` behavior, focused-workspace overflow/return, the Course Flow and Blueprint major-region motion contracts, their keyed cross-view handoff, and reduced-motion final states.
...
- [ ] Open a course and confirm Course Flow settles in as three restrained major regions with a smooth count-aware camera rather than animating every control or node; confirm its unit cards have no decorative top stripe. Then open each lecture and confirm one slim title/health/source header hands directly into one stable full-height map without exposing a preliminary arrangement or second camera snap. Open course health, `Jump to`, Relationships, and Evidence lens on demand; confirm none permanently reduces the graph. Switch Live ↔ Design and run Auto arrange several times, confirming each change shows at most one brief arranging state followed by one settled graph. Confirm the Cross-lecture concept map preserves shared concepts. Repeat with reduced motion enabled and confirm both views appear immediately.
- [ ] Confirm Course Flow shows no redundant selected navigation tab, a selected lecture shows no Blueprint/Assessments/Preview tabs, the header back control returns to Course Flow, and assessment review, learner preview, routing settings, and Sources remain reachable from the compact header without losing the map context.
...
- [ ] Open Course Director before sending a message and confirm there is no separate `Right now` context element, while its short course-specific welcome does not appear as saved history; confirm the larger `New lecture` and Live/Design controls remain visually balanced with the title block, then ask Course Director to modify the course structure and a lecture's internal structure and confirm every operation is accurately scoped and independently reviewable.
...
- Keep Learning Assistant messages only in learner/course-scoped runtime memory for the current login. Close/reopen and client-side navigation retain the transcript; login, logout, full reload, or container-driven reload clears it, with no browser-storage or database transcript writes. A clean transcript renders a presentation-only greeting that names the current concept rather than an empty conversation or separate context label, and that greeting is never stored. Free-text input is classified into an allowlisted intent by the live-verified Agnes 2.5 Flash provider when selected, with GPT-5.4 or the deterministic classifier available as fallback; learner-visible replies come only from deterministic platform guidance, persisted learner evidence, or instructor-reviewed artifacts, so the Assistant must never become a runtime generative tutor. Agnes 1.5 remains a documented catalog option but is not selected while the configured provider returns `model_not_found` for it.
...
- Hybrid-provider contract tests cover Agnes Chat Completions request shape, malformed/schema-invalid output rejection, token telemetry, per-agent provider/key selection, Course Director graph-plan validation, dashboard evidence filtering, allowlisted learner intents, and fallback isolation. An opt-in live smoke script must make one direct no-fallback Agnes call for each selected surface and reject an incorrect graph edit, ungrounded dashboard result, fallback classification, or missing Agnes telemetry before the verified README architecture is updated.
...
- [ ] Open Mastery Map and confirm it renders a readable current/recommended neighborhood of roughly three to five nodes by default; use `Show whole course` and confirm the complete prerequisite graph replaces that focused graph, then use `Focus my path` to return. Confirm real branches and merges remain understandable and persisted remediation/reinforcement/advancement changes are drawn directly on affected nodes and transition edges. Select a changed or blocked node and confirm its highlight plus saved rationale are clear; click empty canvas and confirm the inspector closes.
