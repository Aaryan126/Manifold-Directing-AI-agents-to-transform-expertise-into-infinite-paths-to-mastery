...
- Give the graph a consistent tree grammar: source → topic → concept → teaching clip/check is the sole automatic hierarchy spine, while prerequisite, sequence, remediation, and citation relations render as secondary cross-links without changing ranks. Use distinct accessible node components for every artifact kind, controlled widths with content-aware heights that show complete titles, deterministic top-to-bottom semantic layers, stable sibling order, fixed relation-specific ports, ELK orthogonal edge routes, and sequence-aware crossing minimization. Show the instructional core by default; reveal citations and remediation contextually or through explicit relationship filters. Design mode keeps deliberate saved manual positions and offers a presentation-only Auto arrange action that restores the hierarchy.
