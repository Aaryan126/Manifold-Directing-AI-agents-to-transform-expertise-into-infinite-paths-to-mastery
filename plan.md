...
- Course Flow resolves collisions between saved and newly automatic unit positions. Dense source-less lecture Blueprints use one layout-only root and a compact teach-then-check hierarchy, then pack complete topic subtrees into a balanced multi-row sequence grid while preserving each subtree's internal geometry. The automatic result must have no overlapping nodes and must not change persisted relationship semantics; instructor-saved positions remain authoritative.
