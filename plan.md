...
- Add a synchronized inspector for topic/concept/clip/question/source/relationship records. Clip inspectors play the exact bounded teaching moment in context. In **Design** mode instructors can update inspector fields, connect or reconnect prerequisites, reposition artifacts, and change course sequence through direct or accessible controls. AI-authored changes remain typed proposals with `Accept AI suggestion / Edit manually / Dismiss`; direct teacher edits remain audited and open a private working revision on first save.
...
- Frontend unit/integration tests for complete typed graph construction, artifact-to-node/port mapping, core/all/contextual relation visibility, ELK bend-point consumption, deterministic semantic ordering, saved-position layout, initial fit-to-view, Live/Design switching, topic focus, outline/canvas parity, node movement, in-context clip playback, inspector editing, relationship manipulation, proposal review, and empty/loading/error states.
