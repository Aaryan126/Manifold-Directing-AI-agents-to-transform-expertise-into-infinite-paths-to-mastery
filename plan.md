...
- Add a synchronized inspector and contextual node/edge toolbar for topic/concept/clip/question/source/relationship records. Clip inspectors play the exact bounded teaching moment in context. In **Design** mode instructors can add, update, connect/reconnect, reposition, and safely remove supported artifacts through type-aware direct controls and an accessible outline equivalent. A guided `Add artifact` action offers only semantically valid children/relations for the current selection; invalid targets are explained rather than silently ignored. Edge selection exposes relation meaning, reconnect, and traceable removal.
- Separate learner-facing course order from presentation-only canvas position. A scoped, readable learning-order editor shows neighboring concepts and supports drag/reorder plus keyboard actions; cross-topic moves explicitly ask whether to move or additionally link the concept. Canvas drag/nudge/auto-arrange remain clearly labeled as layout-only and are available for every draggable artifact through accessible controls.
...
- Specialist tests proving one evidence brief or optional post-edit cleanup request can create a one-to-six proposal pack, every proposal has an exact typed before/after diff and citation/evidence provenance, partial failure is visible, and no learner-facing record changes before an atomic Accept/Edit action plus publication.
- Frontend unit/integration tests for complete typed graph construction, artifact-to-node/port mapping, contextual node/edge actions, guided add/remove, deterministic impact preview, scoped learning order, valid-target connection guidance, core/all/contextual relation visibility, ELK bend-point consumption, deterministic semantic ordering, saved-position layout, initial fit-to-view, Live/Design switching, topic focus, outline/canvas parity, node movement, in-context clip playback, inspector editing, relationship manipulation, optional AI cleanup, proposal review, and empty/loading/error states.
