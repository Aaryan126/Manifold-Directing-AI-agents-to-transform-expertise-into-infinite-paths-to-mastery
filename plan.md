...
**Deliverables:**
- Replace the separate published Overview and Course Map destinations with one Blueprint workspace and four modes: **Live** (evidence and priorities), **Design** (direct structural editing), **Learner path** (cohort or selected-learner simulation), and **Revision** (private diffs and review state). Retain Assessments, Preview, and Settings initially as focused bulk/deep editors reachable from Blueprint.
...
- Frontend unit/integration tests for deterministic swimlane construction, shared-concept aliases, artifact summaries, mode/lens switching, semantic focus, outline/canvas parity, constrained sequence/topic drag, inspector editing, relationship manipulation, proposal review, Old Blueprint parity, learner path rendering, and empty/loading/error states.
...
**Human test checklist:**
- [ ] Open a realistic published course and move between Live, Design, Learner path, and Revision without losing the selected topic/concept; confirm the Blueprint makes clips, questions, sources, sequence, and evidence feel like parts of one course system rather than separate tools.
- [ ] In Design mode, add or reconnect a prerequisite, reorder two concepts, edit one inspector field, and confirm each direct edit is audited in a private working revision while the active learner course remains unchanged.
...
- [ ] In Design mode, drag a concept within its lane and confirm the reviewed global sequence changes. Drag a shared concept across topics, exercise both `Also link` and `Move`, and confirm each action creates only a private audited revision while the published learner path remains unchanged.
