...
- Frontend unit/integration tests for complete typed graph construction, artifact-to-node/port mapping, the consolidated Add node menu, four-sided hover ports, relationship-kind choice, valid-target selection/cancellation, contextual node/edge actions, guided add/remove, deterministic impact preview, scoped learning order, core/all/contextual relation visibility, ELK bend-point consumption, deterministic semantic ordering, saved-position layout, single-settle initial fit-to-view, visibly painted semantic-layer node interpolation with a smooth ease-in-out, delayed edge reveal, stable Live/Design switching, reduced-motion completion, topic focus, outline/canvas parity, node movement, in-context clip playback, inspector editing, relationship manipulation, optional AI cleanup, typed Course Director plans, proposal review, and empty/loading/error states.
...
- [ ] Open a course and confirm Course Flow settles in as three restrained major regions with a smooth count-aware camera rather than animating every control or node; then open each lecture and confirm the header title and lecture tabs transition smoothly while the focused Blueprint visibly holds its starting arrangement for a beat and smoothly assembles Source → Topic → Concept → teaching/assessment artifacts into one stable final map, with edges joining only after node motion is underway and no preliminary arrangement or second camera snap. Switch Live ↔ Design and run Auto arrange several times, confirming matching artifacts move continuously from their prior positions and each change settles once. Confirm the Cross-lecture concept map preserves shared concepts. Repeat with reduced motion enabled and confirm both views appear immediately.
