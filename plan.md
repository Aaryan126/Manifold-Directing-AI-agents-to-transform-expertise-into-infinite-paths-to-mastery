...
- [ ] In Live, select a topic, concept, clip, question, and source in turn; confirm only the selected artifact and its directly connected neighbors remain, selecting a neighbor moves the focus, and both the inspector cross and an empty-canvas click restore the complete lecture. Switch to Design and confirm selection no longer hides the wider editing context.
