...
Course creation uses `Source -> Structure -> Assessments -> Adaptation -> Publish`, with `Insights` as a separate post-publication operating workspace. `Structure` owns a unified topic-production queue: topic review, concept coverage, clip generation, and lightweight clip spot-checking appear in one focused topic workspace, while their persisted decisions and audit events remain distinct.
