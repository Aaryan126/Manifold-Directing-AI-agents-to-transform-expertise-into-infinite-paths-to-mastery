...
- 2026-07-20: Topic production now treats missing concept coverage as an inline repair task rather than a dead-end label. Reviewed but unlinked concepts are offered first, instructors can connect one without leaving the topic, and graph creation remains one click away when no suitable concept exists. The selected topic is media-first: compact clip preview and re-cut actions appear before title, summary, timing, and structural actions. This preserves graph-backed routing traceability while making the immediate next step explicit.
...
- 2026-07-20: Unified topic-production implementation and automated verification completed. The queue reports the actionable lifecycle (`Review topic`, `Needs concept`, `Generate/Regenerate clips`, `Clip flagged`, `Ready`); the focused topic editor owns clip generation, preview, flag, and re-cut controls; and Assessments is an isolated third stage. Frontend lint/typecheck, 49 tests, production build, all 7 applicable Playwright journey/WCAG/visual/overflow tests, 88 backend tests, Ruff, MyPy, live Postgres freshness checks, Docker rebuild, and health checks pass. Stage 8 remains pending human confirmation.
