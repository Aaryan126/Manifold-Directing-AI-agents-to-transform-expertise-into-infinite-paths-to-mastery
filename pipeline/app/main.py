...
from app.dependencies import get_course_generation_worker, get_course_intelligence_worker
...
async def lifespan(_: FastAPI) -> AsyncIterator[None]:
    ...
    intelligence_task: asyncio.Task[None] | None = None
    if settings.generation_worker_enabled:
        worker_task = asyncio.create_task(
            get_course_generation_worker().run(stop_event),
            name="course-generation-worker",
        )
    if settings.intelligence_worker_enabled:
        intelligence_task = asyncio.create_task(
            get_course_intelligence_worker().run(stop_event),
            name="course-intelligence-worker",
        )
    try:
        yield
    finally:
        ...
        if worker_task is not None:
            with suppress(asyncio.CancelledError):
                await worker_task
        if intelligence_task is not None:
            with suppress(asyncio.CancelledError):
