...
describe("Course OS presentation", () => {
  ...
  it("packs dense topic subtrees into a readable multi-row hierarchy", () => {
    const nodes = Array.from({ length: 6 }, (_, index) => {
      const topicId = `topic-${index}`;
      return [
        {
          id: topicId,
          logical_id: `logical-${topicId}`,
          kind: "topic" as const,
          title: `Topic ${index + 1}`,
          status: "accepted",
          parent_id: null,
        ...
        {
          id: `concept-${index}`,
          logical_id: `logical-concept-${index}`,
          kind: "concept" as const,
        ...
        {
          id: `clip-${index}`,
          logical_id: `logical-clip-${index}`,
          kind: "clip" as const,
          title: `Teaching moment ${index + 1}`,
          status: "active",
          parent_id: topicId,
          metadata: { sequence_rank: index },
        },
      ];
    }).flat() satisfies BlueprintNode[];
    const edges = Array.from({ length: 6 }, (_, index) => ([
      {
        id: `contains-${index}`,
        source_id: `topic-${index}`,
        target_id: `concept-${index}`,
        semantic_edge_id: `contains-${index}`,
      },
      {
        id: `teaches-${index}`,
        source_id: `concept-${index}`,
        target_id: `clip-${index}`,
        semantic_edge_id: `teaches-${index}`,
      },
    ])).flat();
    const positions = Object.fromEntries(Array.from({ length: 6 }, (_, index) => ([
      [`topic-${index}`, { x: index * 430, y: 0 }],
      [`concept-${index}`, { x: index * 430 + 10, y: 220 }],
      [`clip-${index}`, { x: index * 430 + 20, y: 440 }],
    ])).flat());

    const packed = packDenseBlueprintTopicBranches(nodes, edges, positions);
    const topicRows = new Set(Array.from(
      { length: 6 },
      (_, index) => packed[`topic-${index}`].y,
    ));
    expect(topicRows.size).toBeGreaterThan(1);
    expect(Math.max(...Array.from(
      { length: 6 },
      (_, index) => packed[`topic-${index}`].x,
    ))).toBeLessThan(5 * 430);
    Array.from({ length: 6 }, (_, index) => {
      expect(packed[`concept-${index}`].y - packed[`topic-${index}`].y).toBe(220);
      expect(packed[`clip-${index}`].y - packed[`concept-${index}`].y).toBe(220);
    });
    ...
    rectangles.forEach((rectangle, index) => {
      rectangles.slice(index + 1).forEach((candidate) => {
        expect(
          rectangle.x < candidate.x + candidate.width
          && rectangle.x + rectangle.width > candidate.x
          && rectangle.y < candidate.y + candidate.height
          && rectangle.y + rectangle.height > candidate.y,
        ).toBe(false);
      });
    });
  });
