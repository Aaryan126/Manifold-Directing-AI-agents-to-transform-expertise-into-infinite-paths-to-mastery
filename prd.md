...
1. Instructor enters the Teacher Command Center and opens a full-width Course Director conversation to submit the first lecture file or link, with an optional audience/outcome brief. Merely opening this surface does not add an untitled shell to `Your courses`.
2. The focused conversation remains the primary surface while a durable server-side agent run transcribes the source and prepares a complete private draft: a reviewable course title, an initial Course Flow lecture unit, topics, concept graph, clips, assessments, remediation, routing recommendations, and quality checks. The instructor may leave and resume later.
...
- The default course workspace is a revision-aware **Course Flow** above Blueprint. Its visible graph contains only lecture, standalone quiz, and assignment units, arranged left-to-right inside optional neutral module/week swimlanes. `next`, `requires`, and `assesses` relationships define learner progression and coverage without exposing lecture-internal topics, concepts, clips, or questions. Selecting a lecture opens Blueprint focused on that lecture while retaining a Whole course view for shared concepts and cross-lecture prerequisites. Selecting a quiz or assignment opens the same underlying reviewed artifact in its focused editor. Course Flow and Blueprint share Live/Design semantics, undo, typed relationship validation, private revision persistence, and atomic publication.
