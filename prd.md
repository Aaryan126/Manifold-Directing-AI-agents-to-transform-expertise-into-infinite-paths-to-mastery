...
- Published course structure and evidence meet in one **Blueprint** workspace rather than separate Overview and Course Map destinations. The published evidence view is the implicit default rather than a `Live` mode tab. `Edit Blueprint / Done editing` explicitly controls direct private structural editing; `Preview learner journey` opens a dedicated evidence-backed path simulation; and `Review changes · N` appears only when a real private diff exists and opens a live-versus-working review. The selected topic/concept and Course/Dependencies/Focused lens remain stable across those actions. The whole-course view is a deterministic, responsive set of topic swimlanes containing sequenced concept bundles; clips, questions, remediation, sources, and typed relations progressively appear in topic/concept focus instead of competing in an all-node wall. Concepts shared across topics appear as synchronized visual aliases. A separate dependency lens renders unique concepts and prerequisite relations with fixed-port orthogonal ELK layout. Edit-mode drag reorders concepts within the reviewed global sequence; cross-topic drops explicitly ask whether to add or move the topic link, then write only to a private audited revision. React Flow remains the interaction layer. The prior free-form all-artifact implementation remains fully functional as `Old Blueprint` until the revised human checklist is confirmed, and its saved positions remain isolated from the structured layout.
...
- `Edit Blueprint` allows direct instructor editing of inspector fields, explicit concept sequence, and prerequisite connections. Direct teacher edits are audited and automatically open a private working revision; AI-authored edits remain proposals. `Preview learner journey` presents the same reviewed graph, sequence, content, assessments, mastery state, and routing evidence for a cohort or selected learner. `Review changes · N` shows exact private live-versus-working diffs and is hidden when there is nothing to review. Assessments, Learner Preview, and Settings remain initially as focused bulk/deep editors reachable from Blueprint rather than independent product silos.
...
- The default Blueprint separates course-design health (coverage, structure, source alignment, assessment/clip readiness) from learner-evidence health (reach, correctness, confidence, mastery, remediation, and trends), so a new course remains useful without fabricated learner data.
- `Edit Blueprint` uses the same selected structure as the default evidence view for persisted graph/sequence manipulation, avoiding a separate diagnostic map that drifts from its editor. Every canvas operation has an accessible outline/inspector equivalent.
