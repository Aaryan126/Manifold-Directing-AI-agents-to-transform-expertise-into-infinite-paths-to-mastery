...
| Interaction motion | **Motion for React** | Shared-layout projection powers the instructor and learner assistant capsule-to-dock transition. The same small motion vocabulary provides page settles, major-section cascades, and skeleton-to-content handoffs across the application, with maintained presence/layout measurement and a user reduced-motion fallback instead of bespoke animation systems. |
