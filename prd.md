...
| Interaction motion | **Motion for React** | Shared-layout projection powers the instructor and learner assistant capsule-to-dock transition and Blueprint Live's double-buffered direct-neighborhood handoff. The same small motion vocabulary provides page settles, major-section cascades, skeleton-to-content handoffs, the keyed Course Flow-to-Blueprint presence transition, and stable-map reveals; React Flow remains responsible for viewport calculation, ELK positions, and the authoritative fit. Initial/mode/topic/Auto-arrange intermediates stay concealed, while transient Live node focus keeps the prior map visible until shared retained nodes can spring into the ready projection. All paths share a user reduced-motion fallback instead of bespoke animation systems. |
