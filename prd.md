...
| Interaction motion | **Motion for React + D3 timing/interpolation** | Motion shared-layout projection powers the instructor and learner assistant capsule-to-dock transition and the shared page/section vocabulary. Blueprint node-position changes use open-source `d3-timer` and `d3-interpolate` over ELK’s final coordinates after React Flow establishes the final viewport; intermediate layouts remain concealed, edges join late, and all paths share a user reduced-motion fallback instead of competing bespoke animation systems. |
