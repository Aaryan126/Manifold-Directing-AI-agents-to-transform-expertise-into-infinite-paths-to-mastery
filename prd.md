...
| Interaction motion | **Motion for React** | Shared-layout projection powers the instructor and learner assistant capsule-to-dock transition. The same small motion vocabulary provides page settles, major-section cascades, skeleton-to-content handoffs, and the keyed Course Flow-to-Blueprint presence transition; React Flow's own animated fit handles graph-camera arrival. All paths share a user reduced-motion fallback instead of bespoke animation systems. |
