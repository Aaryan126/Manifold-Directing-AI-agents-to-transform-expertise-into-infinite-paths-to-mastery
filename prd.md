...
- Design mode allows direct instructor editing of inspector fields, explicit concept sequence, saved node positions, and prerequisite connections. Direct teacher edits are audited and automatically open a private working revision; AI-authored edits remain proposals. Clip inspectors play the exact bounded teaching moment so an instructor can verify the artifact without leaving graph context. Learner experience remains in Preview and the learner workspace; exact live-versus-working comparison remains in the publication/revision review flow. Assessments, Learner Preview, and Settings remain focused bulk/deep editors reachable from Blueprint rather than independent product silos.
...
- Blueprint Design mode uses the same selected structure as Live mode for persisted graph/sequence manipulation, avoiding a separate diagnostic map that drifts from its editor. Every canvas operation has an accessible outline/inspector equivalent.
