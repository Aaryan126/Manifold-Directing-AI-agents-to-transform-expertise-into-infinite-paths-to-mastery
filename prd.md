...
- Design mode is a type-aware course authoring surface rather than a generic graph editor. Selecting a topic, concept, clip, question, source, or relationship exposes only valid contextual actions; a guided `Add artifact` action, explicit relationship meanings/targets, edge reconnect/remove, safe removal impact previews, and equivalent outline/inspector controls make add/edit/remove discoverable without relying on connector handles. Learner-facing course order is edited in a scoped sequence surface with neighboring concepts and keyboard controls, while canvas drag/nudge/auto-arrange are labeled as presentation-only. Direct teacher edits are audited and automatically open a private working revision.
